...
func TestContext(t *testing.T) {
	Convey("Given a new pool", t, func() {
		...
		Convey("It starts with given creationIndex", func() {
			So(NewContext(NumComponents, 42).CreateEntity().ID(), ShouldEqual, 42)
		})
		...
		Convey("When entities are cached", func() {
			...
			Convey("It returns new entity", func() {
				...
				p.DestroyEntity(e)
				p.CreateEntity()
				entityFromContext := p.CreateEntity()
				So(entityFromContext.HasComponent(ComponentA), ShouldBeFalse)
				So(entityFromContext, ShouldNotEqual, e)
			})
		...
		Convey("When entities are retrieved", func() {
			...
			Convey("Its matcher2group dispatches OnEntityWillBeRemoved, OnEntityRemoved and OnEntityAdded when replacing components", func() {
				g := p.Group(matcher)
...
func BenchmarkContextCreateGroup(b *testing.B) {
	p := NewContext(NumComponents, 0)
